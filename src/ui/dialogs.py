...
SETTINGS_AUTHOR_URL = "https://github.com/edgarp9"
...
class SettingsDialog(tk.Toplevel):
    ...
    def _open_author_link(self) -> None:
        try:
            opened = webbrowser.open_new_tab(SETTINGS_AUTHOR_URL)
        except Exception:
            LOGGER.exception("Failed to open settings author link.")
            return
        if not opened:
            LOGGER.warning(
                "Browser did not report opening settings author link. url=%s",
                SETTINGS_AUTHOR_URL,
            )
