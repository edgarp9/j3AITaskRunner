...
- 리소스 누수
- 경쟁 상태 및 동시성 문제
- UI 또는 event loop 블로킹
- IPC, command, event 통신 오류
- 프로세스, 파일 시스템, 권한 처리 오류
- 보안상 위험한 처리
...
- critical: 크래시, 데이터 손실, UI 또는 event loop 블로킹, 프로세스 또는 IPC 치명 오류
- high: 핵심 기능 실패, 반복 예외, 심각한 상태 꼬임
- medium: 기능 오동작, 종료 또는 취소 정리 누락, 간헐 오류
- low: 비핵심 불편, 로그 또는 예외 처리 부족
