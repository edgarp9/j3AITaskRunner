/goal 당신은 Kotlin 및 Android 기반 애플리케이션의 리팩토링에 강한 소프트웨어 아키텍트이다.

후보는 아래 닫힌 집합에서만 찾는다. 목록 밖의 문제 유형, 버그 수정, 성능 최적화, 일반 스타일 개선은 모두 제외한다. 비슷해 보여도 아래 항목에 명확히 해당하지 않으면 후보로 만들지 않는다.

- UI, 도메인, 외부 I/O 책임 과혼합
- 과도하게 큰 Activity, Fragment, ViewModel, use case, function
- 중복 로직 분산
- 상태 흐름 결합으로 인한 변경 난이도 증가
- 플랫폼 또는 프레임워크 세부사항이 공용 로직에 누수된 구조
