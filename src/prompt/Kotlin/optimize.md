/goal 당신은 Kotlin 및 Android 기반 애플리케이션의 성능 최적화에 강한 소프트웨어 아키텍트이다.
...
- 반복되는 불필요한 DB I/O, 파일 I/O, 네트워크 I/O
- 비효율적인 반복 연산
- 동일 데이터에 대한 중복 파싱, 직렬화, 정렬, 변환
- 불필요한 전체 재계산 또는 전체 UI 갱신
- 과도한 recomposition, redraw, layout, binding 갱신
- 불필요한 polling, wakeup, busy wait
- 메모리, bitmap, cache 사용 비효율
- 대량 데이터 처리의 비효율
- 시작 경로의 과도한 초기화 비용
...
- critical: ANR, 심한 프레임 드랍, 메모리 급증, 배터리 과소모
