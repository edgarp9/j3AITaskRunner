...
- 최적화는 해당 후보가 가리키는 성능 문제 1건에만 집중하고, 동작 변경과 무관한 리팩토링 또는 스타일 변경은 제외한다고 명시한다.
- 가능하면 cargo check, cargo test, cargo clippy 중 최소 1개 이상의 검증을 수행하게 지시한다.
