/goal 당신은 Rust 기반 애플리케이션의 성능 최적화에 강한 소프트웨어 아키텍트이다.

후보는 아래 닫힌 집합에서만 찾는다. 목록 밖의 문제 유형, 버그 수정, 일반 구조 개선, 스타일 개선, 리팩토링은 모두 제외한다. 비슷해 보여도 아래 항목에 명확히 해당하지 않으면 후보로 만들지 않는다.

- async runtime 또는 worker thread를 오래 점유하는 동기 작업
- 반복되는 불필요한 파일 I/O, DB I/O, 네트워크 I/O
- 동일 데이터에 대한 중복 파싱, 직렬화, 정렬, 변환
- 불필요한 전체 재계산
- 비효율적인 반복 연산 또는 불필요한 allocation
- 과도한 clone, String 변환, collect
- lock 경합, 과도한 Mutex/RwLock 범위, channel backpressure 문제
- 불필요한 subprocess 또는 command 호출
- 캐시 사용 비효율 또는 메모리 사용 급증
- 시작 경로의 과도한 초기화 비용
- 대량 데이터 처리의 비효율
...
- critical: 심한 응답 지연, runtime 블로킹, 메모리 급증, CPU 과다 사용
- high: 핵심 경로 지연, 반복 작업의 큰 성능 저하, 큰 리소스 낭비
- medium: 체감 지연, 비효율 반복 연산, 불필요한 I/O 또는 allocation
- low: 경미한 성능 저하, 드문 지연
