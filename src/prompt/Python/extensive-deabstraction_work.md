...
규칙:
- 각 프롬프트는 Python/Tkinter De-abstraction 작업만 지시한다.
...
- 단순한 직접 호출, 구체 타입, 명확한 함수 구조를 선호하되 UI, app, infra 책임 경계를 흐리지 않게 지시한다.
...
- 가능하면 기존 동작 보존을 확인하는 `python -m compileall .`, `python -m unittest`, `python -m pytest`, `ruff check .` 중 최소 1개 이상의 검증을 수행하게 지시한다.
